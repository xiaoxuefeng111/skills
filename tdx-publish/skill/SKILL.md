...
| **带打包** | `/tdx-publish --pack CCGR-native-beta.txt tdxCore master abc123 6.9.5` |
| **批量** | `/tdx-publish --input records.json` |

### 参数格式

```
/tdx-publish [--pack] 模板文件 模块 分支 commit 版本 [模块 分支 commit 版本]...
```
...
如果 AAR 已经发布，只需要打包，使用 `/tdx-pack` 技能：
```
/tdx-pack
```
然后输入打包配置和模块版本即可。
